package com.xhsj.customview.materialdesign.nestedscroll.view;
...
//                consumed[1] = dy;
                if(dispatchNestedPreScroll(dx,dy,consumed,null)){
                    Log.i("-----onMeasure11","dy: " + dy + ", cosumed: " + consumed[1]);
                    dy -= consumed[1];
                    if(dy == 0){
                        Log.i("-----onMeasure22","dy: " + dy);
                        return true;
                    }
                }else{
                    Log.i("----onMeasure33","scrollBy: " + dy);
                    scrollBy(0,dy);
                }
                break;
            }
